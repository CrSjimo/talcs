            ...
            for (auto src : sourceDict.keys()) {
                ...
                if(routeChannels) {
                    if(routeCnt >= channelCount / 2)
                        break;
                    readData.buffer->addSampleRange(routeCnt * 2, readData.startPos, readLength, tmpBuf, 0, 0, gainLeftRight.first);
                    readData.buffer->addSampleRange(routeCnt * 2 + 1, readData.startPos, readLength, tmpBuf, 1, 0, gainLeftRight.second);
                    routeCnt++;
                } else {
