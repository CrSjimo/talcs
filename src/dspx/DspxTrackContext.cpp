...
namespace talcs {
    ...
    DspxAudioClipContext *DspxTrackContext::addAudioClip(int id) {
        ...
        d->clips.insert(id, {clip, clipView});
    ...
    void DspxTrackContext::removeAudioClip(int id) {
        ...
        d->clips.remove(id);
        d->clipSeries->removeClip(clip.second);
    }

    QList<DspxAudioClipContext *> DspxTrackContext::clips() const {
        Q_D(const DspxTrackContext);
        QList<DspxAudioClipContext *> list;
        list.reserve(d->clips.size());
        for (const auto &[clip, _] : d->clips.values()) {
            list.append(clip);
        }
        return list;
    }
